...
export function unlockQuest(quest: any) {
  const [effect] = quest.children;
  applyEffects(effect);
  playOpening(quest);
  return;
}

function applyEffects(node: any) {
  var child = node.children[0];
  var locations = flatten(child);
  locations.filter(node => node.tagName != Tagnames.LOCATION).forEach(processEffectsLocation);
}

export function playOpening(quest: object) {
